...
if (@$_REQUEST['hardcron']=='1')
  B2NWC_cron_job_worker (true);

//===========================================================================
// '$hardcron' == true if job is ran by Cpanel's cron job.

function B2NWC_cron_job_worker ($hardcron=false)
{
  global $wpdb;
  ...
  $query =
    "SELECT * FROM `$b2n_payments_table_name`
      WHERE
      (
  ...
  if (is_array($rows_for_balance_check))
  {
  	...
  	foreach ($rows_for_balance_check as $row_for_balance_check)
  	{
		  ...
		  $address_meta = B2NWC_unserialize_address_meta(@$row_for_balance_check['address_meta']);
		  ...
		  // Retrieve current balance at address considering required confirmations number and api_timemout value.
			$address_request_array['b2n_payment_id'] = $row_for_balance_check['b2n_payment_id'];
      ...
      $address_request_array['block_index'] = $row_for_balance_check['index_in_wallet'];
        ...
        $current_time = time();
        $query =
          "UPDATE `$b2n_payments_table_name`
             SET
        ...
        if ($balance_info_array['balance'] > 0)
        {
          ...
          B2NWC__log_event (__FILE__, __LINE__, "Cron job: NOTE: Detected non-zero balance at address: '{$row_for_balance_check['b2n_address']}, Payment ID = {$row_for_balance_check['b2n_payment_id']}, order ID = '{$last_order_info['order_id']}'. Detected balance ='{$balance_info_array['balance']}'.");

          if ($balance_info_array['balance'] < $last_order_info['order_total'])
          {
            B2NWC__log_event (__FILE__, __LINE__, "Cron job: NOTE: balance at address: '{$row_for_balance_check['b2n_address']}, Payment ID = {$row_for_balance_check['b2n_payment_id']}' (B2N '{$balance_info_array['balance']}') is not yet sufficient to complete it's order (order ID = '{$last_order_info['order_id']}'). Total required: '{$last_order_info['order_total']}'. Will wait for more funds to arrive...");
          }
        }
		         ...
		         array (
		            'orders' =>
		               array (
		                  ...
		                  array (
		                     'order_id'     => $order_id,
		                     'order_total'  => $order_total_in_b2n,
	        ...
	        B2NWC__log_event (__FILE__, __LINE__, "Cron job: NOTE: Full payment for order ID '{$last_order_info['order_id']}' detected at address: '{$row_for_balance_check['b2n_address']}, Payment ID = {$row_for_balance_check['b2n_payment_id']}' (B2N '{$balance_info_array['balance']}'). Total was required for this order: '{$last_order_info['order_total']}'. Processing order ...");
	        ...
	        B2NWC__process_payment_completed_for_order ($last_order_info['order_id'], $balance_info_array['balance']);

	        // Update address' record
	        $address_meta_serialized = B2NWC_serialize_address_meta ($address_meta);
          ...
          //
	        $query =
	          "UPDATE `$b2n_payments_table_name`
	            ...
	            WHERE `id`='$row_id';";
	        $ret_code = $wpdb->query ($query);
	        B2NWC__log_event (__FILE__, __LINE__, "Cron job: SUCCESS: Order ID '{$last_order_info['order_id']}' successfully completed.");

		    }
		  }
		  else
		  {
		    B2NWC__log_event (__FILE__, __LINE__, "Cron job: Warning: Cannot retrieve balance for address: '{$row_for_balance_check['b2n_address']} Payment ID: {$row_for_balance_check['b2n_payment_id']}: " . $balance_info_array['message']);
		  }
		  //..//
		}
	}
  ...
  //         B2NWC__log_event (__FILE__, __LINE__, "Cron job: NOTE: Detected LATE PAYMENT non-zero balance at address: '{$row_for_balance_check['b2n_address']}, Payment ID = {$row_for_balance_check['b2n_payment_id']}, order ID = '{$last_order_info['order_id']}'. Detected balance ='{$balance_info_array['balance']}'.");
  ...
  //           // Apply exchange rate multiplier only for stores with non-B2Ncoin default currency.
  //           $order_total_in_b2n = $order_total_in_b2n;

  //         $order_total_in_b2n   = sprintf ("%.8f", $order_total_in_b2n);

  //         $difference = $order_total_in_b2n - $last_order_info['order_total'];
  ...
  //                                 __('New Order Total: ', 'woocommerce') . $order_total_in_b2n . ' '.
  //                                 __('Difference: ', 'woocommerce') . $order_total_in_b2n . ' (' . $diffincur . ' '. get_woocommerce_currency() .')'
  //                               );

  //         if ($balance_info_array['balance'] < $last_order_info['order_total'])
  //         {
  //           B2NWC__log_event (__FILE__, __LINE__, "Cron job: NOTE: LATE PAYMENT balance at address: '{$row_for_balance_check['b2n_address']}, Payment ID = {$row_for_balance_check['b2n_payment_id']}' (B2N '{$balance_info_array['balance']}') is not yet sufficient to complete it's order (order ID = '{$last_order_info['order_id']}'). Total required: '{$last_order_info['order_total']}'. Will wait for more funds to arrive...");
  //         }
  //       }
  ...
  //         // Last order was fully paid! Complete it...
  //         B2NWC__log_event (__FILE__, __LINE__, "Cron job: NOTE: Full LATE PAYMENT for order ID '{$last_order_info['order_id']}' detected at address: '{$row_for_balance_check['b2n_address']}, Payment ID = {$row_for_balance_check['b2n_payment_id']}' (B2N '{$balance_info_array['balance']}'). Total was required for this order: '{$last_order_info['order_total']}'. Processing order ...");
  ...
  //             WHERE `id`='$row_id';";
  //         $ret_code = $wpdb->query ($query);
  //         B2NWC__log_event (__FILE__, __LINE__, "Cron job: SUCCESS LATE PAYMENT: Order ID '{$last_order_info['order_id']}' successfully completed.");
  //       }
  //     }
  //     else
  //     {
