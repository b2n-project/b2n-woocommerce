...
function B2NWC__generate_new_B2Ncoin_payment_id ($b2nwc_settings=false, $order_info)
{
  ...
  $address_request_array['b2n_address'] = $b2n_address;
  ...
  $query = "INSERT INTO `$b2n_payments_table_name` (`b2n_address`, `b2n_payment_id`, `origin_id`, `index_in_wallet`, `status`, `total_received_funds`, `received_funds_checked_at`, `assigned_at`, `address_meta`) VALUES ('$b2n_address', '$new_b2n_payment_id', 'none', '$next_key_index', '$status', '$funds_received', '$received_funds_checked_at_time', '$assigned_at_time', '$address_meta_serialized');";
  $ret_code = $wpdb->query($query);

  return $new_b2n_payment_id;
}
//===========================================================================

//===========================================================================
// Function makes sure that returned value is valid array
function B2NWC_unserialize_address_meta ($flat_address_meta)
{
...
function B2NWC_serialize_address_meta ($address_meta_arr)
{
   return B2NWC__safe_string_escape(serialize($address_meta_arr));
}
//===========================================================================
...
//===========================================================================
// Returns:
//    success: number of currency units (dollars, etc...) would take to convert to 1 B2Ncoin, ex: "15.32476".
//    failure: false
//
...
function B2NWC__get_exchange_rate_per_B2Ncoin ($currency_code, $rate_retrieval_method = 'getfirst', $get_ticker_string=false)
{
  ...
  if (!$exchange_multiplier)
	...
	$ticker_string = "<span style='color:#222;'>According to your settings (including multiplier), current calculated rate for 1 B2Ncoin (in {$currency_code})={{{EXCHANGE_RATE}}}</span>";
...
//===========================================================================
function B2NWC__function_not_exists ($fname) { return !function_exists($fname); }
//===========================================================================
...
*/
function B2NWC__file_get_contents ($url, $return_content_on_error=false, $timeout=60, $user_agent=FALSE, $is_post=false, $post_data="")
{
		...
		foreach ($post_data as $k => $v)
			{
				...
				if (is_object($safetied))
					$safetied = B2NWC__object_to_array($safetied);
				if (is_array($safetied))
				{
					$safetied = serialize($safetied);
					$safetied = $p . str_replace('=', '_', B2NWC__base64_encode($safetied));
...
//===========================================================================
function B2NWC__object_to_array ($object)
{
	if (!is_object($object) && !is_array($object))
    return $object;
  return array_map('B2NWC__object_to_array', (array) $object);
}
//===========================================================================

//===========================================================================
// Credits: http://www.php.net/manual/en/function.mysql-real-escape-string.php#100854
function B2NWC__safe_string_escape ($str="")
{
   $len=strlen($str);
   $escapeCount=0;
   $targetString='';
...
//===========================================================================
// Syntax:
//    B2NWC__log_event (__FILE__, __LINE__, "Hi!");
//    B2NWC__log_event (__FILE__, __LINE__, "Hi!", "/..");
//    B2NWC__log_event (__FILE__, __LINE__, "Hi!", "", "another_log.php");
function B2NWC__log_event ($filename, $linenum, $message, $prepend_path="", $log_file_name='__log.php')
{
   ...
   if ($fhandle)
      {
      @fwrite ($fhandle, "\r\n// " . $_SERVER['REMOTE_ADDR'] . '(' . $_SERVER['REMOTE_PORT'] . ')' . ' -> ' . date("Y-m-d, G:i:s T") . "|" . B2NWC_VERSION . "/" . B2NWC_EDITION . "|$filename($linenum)|: " . $message . $logfile_tail);
      @fclose ($fhandle);
      }
}
//===========================================================================
...
//===========================================================================
function B2NWC__send_email ($email_to, $email_from, $subject, $plain_body)
{
   $message = "
   <html>
   <head>
...
function B2NWC__base64_decode($input)
{
...
function B2NWC__base64_encode($data)
{
    ...
    $o1 = B2NWC_charCodeAt($data, $i++);
    $o2 = B2NWC_charCodeAt($data, $i++);
    $o3 = B2NWC_charCodeAt($data, $i++);
    $bits = $o1 << 16 | $o2 << 8 | $o3;
    $h1 = $bits >> 18 & 0x3f;
    $h2 = $bits >> 12 & 0x3f;
    $h3 = $bits >> 6 & 0x3f;
    ...
    $tmp_arr[$ac++] = B2NWC_charAt($b64, $h1).B2NWC_charAt($b64, $h2).B2NWC_charAt($b64, $h3).B2NWC_charAt($b64, $h4);
    } while ($i < strlen($data));
    $enc = implode($tmp_arr, '');
    $r = (strlen($data) % 3);
...
function B2NWC_charCodeAt($data, $char) {
    return ord(substr($data, $char, 1));
}

function B2NWC_charAt($data, $char) {
