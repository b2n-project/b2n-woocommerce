...
//function B2NWC__render_advanced_settings_page ()  { B2NWC__render_settings_page   ('advanced'); }
//===========================================================================
...
HHHH;
  ...
  $gateway_valid_for_use = B2NWC__is_gateway_valid_for_use($gateway_status_message);
  if (!$gateway_valid_for_use)
  {
    ...
    "B2Ncoin Payment Gateway is NOT operational (try to re-enter and save settings): " . $gateway_status_message .
    '</p>';
  }
  else
  {
    ...
    "B2Ncoin Payment Gateway is operational" .
  ...
  if (!$currency_code || $currency_code=='B2N')
    $currency_code = 'USD';

  $exchange_rate_message =
    '<p style="border:1px solid #DDD;padding:5px 10px;background-color:#cceeff;">' .
    B2NWC__get_exchange_rate_per_B2Ncoin ($currency_code, 'getfirst', true) .
    '</p>';
   ...
   switch ($menu_page_name)
      {
      case 'general'     :
        echo  B2NWC__GetPluginNameVersionEdition();
        echo  $gateway_status_message . $exchange_rate_message;
        B2NWC__render_general_settings_page_html();
        break;
        ...
        <tr valign="top">
          ...
          <td>
            ...
            <p class="description">
              ...
              May be useful to compensate for market volatility or for merchant's loss to fees when converting B2Ncoins to local currency,
              or to encourage customer to use B2Ncoins for purchases (by setting multiplier to < 1.00 values).
            </p>
          </td>
        </tr>
        ...
        <tr valign="top">
            ...
            <td>
              ...
              <p class="description">
                ...
                <?php echo '<tt style="background-color:#FFA;color:#B00;padding:0px 6px;">wget -O /dev/null ' . $g_B2NWC__cron_script_url . '?hardcron=1</tt>'; ?>
                <br /><b style="color:red;">NOTE:</b> Cron jobs <b>might not work</b> if your site is password protected with HTTP Basic auth or other methods. This will result in WooCommerce store not seeing received payments (even though funds will arrive correctly to your B2Ncoin addresses).
                ...
                <br />For secure, fast hosting service optimized for wordpress and 100% compatibility with WooCommerce and B2Ncoin we recommend <b><a href="http://hostrum.com/" target="_blank">Hostrum Hosting</a></b>.
              </p>
            </td>
        </tr>
